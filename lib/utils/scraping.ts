...
export const constructorFullNames: Record<string, string> = {
  'RED': 'Red Bull Racing',
  'MCL': 'McLaren',
  'FER': 'Ferrari',
  'MER': 'Mercedes',
...
// Driver name mapping
export const driverFullNames: Record<string, string> = {
  'NOR': 'Lando Norris',
  'VER': 'Max Verstappen',
  'RUS': 'George Russell',
  'PIA': 'Oscar Piastri',
...
// Add new interfaces for race calendar
interface Race {
  round: string;
  date: string;
  country: string;
...
export async function scrapeConstructorStandings(): Promise<ScrapedData> {
  ...
  try {
    ...
    const standings = await page.evaluate((constructorNames: Record<string, string>) => {
      ...
      return Array.from(rows)
        ...
        .filter((item): item is ConstructorStanding => item !== null);
...
export async function scrapeDriverStandings(): Promise<ScrapedData> {
  let browser: Browser | undefined;
  try {
    ...
    const standings = await page.evaluate((driverNames: Record<string, string>, driverTeams: Record<string, string>) => {
      ...
      return Array.from(rows)
        ...
        .filter((item): item is DriverStanding => item !== null);
...
export async function writeConstructorsFile(data: ScrapedData): Promise<void> {
...
export async function writeDriversFile(data: ScrapedData): Promise<void> {
...
// Add race calendar scraping function
export async function scrapeF1Calendar(): Promise<Race[]> {
  let browser: Browser | undefined;
  try {
...
// Add race calendar file writing function
export async function writeRacesFile(data: Race[]): Promise<void> {
...
// Update the GET handler to handle all three routes
